...
import com.test.RongYun.activity.LoginActivity;
import com.test.RongYun.base.MyVolley;
import com.test.RongYun.db.BlackList;
import com.test.RongYun.db.BlackListDao;
import com.test.RongYun.db.DBManager;
import com.test.RongYun.db.Friend;
import com.test.RongYun.db.FriendDao;
import com.test.RongYun.db.GroupMember;
import com.test.RongYun.db.GroupMemberDao;
import com.test.RongYun.db.Groups;
import com.test.RongYun.db.GroupsDao;
import com.test.RongYun.db.UserInfoBean;
import com.test.RongYun.protocol.GetAllFriendListProtocol;
...
public class SealUserInfoManager implements OnDataListener {
     ...
     */
    public void openDB() {
       Log.e(TAG, "SealUserInfoManager openDB");
...
//            List<UserRelationshipResponse.ResultEntity> list = userRelationshipResponse.getDataList();
...
//        }
//        return false;
        GetAllFriendListProtocol getAllFriendListProtocol = new GetAllFriendListProtocol();
        HashMap<String,String> map=new HashMap<>();
        String url = getAllFriendListProtocol.getApiFun();
        ...
        MyVolley.uploadNoFile(MyVolley.POST, url, map, new MyVolley.VolleyCallback() {
            ...
            public void dealWithJson(String address, String json) {
                ...
                Gson gson = new Gson();
                final UserRelationshipResponse userRelationshipResponse = gson.fromJson(json, UserRelationshipResponse.class);
                LogUtils.e("userRelationshipResponse:" + userRelationshipResponse.toString());
    ...
    private List<Friend> pullFriends() throws HttpException {
        ...
        if (userRelationshipResponse != null && userRelationshipResponse.getCode() == 0) {
            List<UserRelationshipResponse.ResultEntity> list = userRelationshipResponse.getDataList();
            if (list != null && list.size() > 0) {
                syncDeleteFriends();
    ...
    private List<Friend> addFriends(final List<UserRelationshipResponse.ResultEntity> list) {
        if (list != null && list.size() > 0) {
            ...
            for (UserRelationshipResponse.ResultEntity resultEntity : list) {
                if (resultEntity.getStatus() == 20 && resultEntity != null) {
                    Friend friend = new Friend(
                            resultEntity.getFriendUserId(),
                            resultEntity.getNameRemark(),
                            Uri.parse(resultEntity.getPortraitUri()),
                            resultEntity.getNameRemark(),
                            null, null, null, null,
                            CharacterParser.getInstance().getSpelling(resultEntity.getNameRemark()),
                            CharacterParser.getInstance().getSpelling(resultEntity.getNameRemark()));
                    if (friend.getPortraitUri() == null || TextUtils.isEmpty(friend.getPortraitUri().toString())) {
        ...
        if (result != null) {
            if (requestCode == GET_TOKEN) {
                ...
                if (tokenResponse.getCode() == 200) {
                    ...
                    SharedPrefrenceUtils.setString(mContext,"authorization",token);
