...
import com.test.RongYun.widget.SinglePopWindow;
...
public class UserDetailActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initData() {
        ...
        if (mFriend != null) {
            ...
            ImageLoader.getInstance().displayImage(portraitUri, mUserPortrait, BaseApplication.getOptions());
        }
        ...
        if (!TextUtils.isEmpty(mFriend.getUserId())) {
            String mySelf = SharedPrefrenceUtils.getString(mContext,"userid");
        ...
        if (result != null) {
            switch (requestCode) {
                ...
                case SYN_USER_INFO:
                    ...
                    if (userInfoByIdResponse.getCode() == 0 && userInfoByIdResponse.getData() != null &&
                            mFriend.getUserId().equals(userInfoByIdResponse.getData().getUserId())) {
                        String nickName = userInfoByIdResponse.getData().getName();
                        String portraitUri = userInfoByIdResponse.getData().getPortraitUri();
                        if (hasNickNameChanged(nickName) || hasPortraitUriChanged(portraitUri)) {
                            ...
                            UserInfo userInfo = new UserInfo(userInfoByIdResponse.getData().getUserId(), nickName, Uri.parse(portraitUri));
    ...
    public void runAddUser() {
        ...
        map.put("friendUserId",  mFriend.getUserId());
        map.put("remark",addMessage);
