...
import com.test.RongYun.activity.MainActivity;
import com.test.RongYun.activity.NewFriendListActivity;
import com.test.RongYun.activity.UserDetailActivity;
import com.test.RongYun.db.Friend;
import com.test.RongYun.db.GroupMember;
import com.test.RongYun.db.Groups;
import com.test.RongYun.message.module.SealExtensionModule;
import com.test.RongYun.response.ContactNotificationMessageData;
...
//        RongMessageItemLongClickActionManager.getInstance().addMessageItemLongClickAction(action, 1);
    ...
    public void onChanged(ConnectionStatus connectionStatus) {
        ...
        if (connectionStatus.equals(ConnectionStatus.KICKED_OFFLINE_BY_OTHER_CLIENT)) {
            ...
            final String cacheToken = SharedPrefrenceUtils.getString(mContext,"token");
    ...
    private void quit(boolean isKicked) {
        Log.d(TAG, "quit isKicked " + isKicked+"   mContext:"+RongIM.getInstance());
        SharedPreferences.Editor editor = mContext.getSharedPreferences("config", Context.MODE_PRIVATE).edit();
        if (!isKicked) {
            editor.putBoolean("exit", true);
        }
        SharedPrefrenceUtils.setString(mContext,"authorization","");
